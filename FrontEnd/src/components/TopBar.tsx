...
export function TopBar({ accountType, onAccountTypeChange, session, onLogout }: TopBarProps) {
  ...
  const activeAccountLabel = isDemoMode ? "Demo Account" : "Real Account";
  ...
  const activeAccountSubline = isDemoMode
    ? demoInitialized
      ? `${demoHoldingsCount} saved assets for ${session.username}. Seeded ${formatDateTime(demoAccount?.seededAt)}.`
      : "Choose your starting capital and target asset mix. Your demo account is saved per user in the database."
    : connectionConnected
      ? data?.connection.message ?? "Live account is connected and synced to your stored credentials."
      : "Connect Binance in Settings to load your live portfolio for this user.";
  ...
  return (
    <>
      <header className="page-enter h-14 flex items-center justify-between gap-4 px-5 border-b border-border bg-card/80 backdrop-blur-xl">
        {/* Left: Mode toggle */}
        <div className="flex items-center gap-4">
          <div className="relative inline-flex items-center rounded-full border border-border bg-background/60 p-0.5">
            <span
              className={cn(
            ...
            <button
              ...
              className={cn(
                "relative z-10 w-20 rounded-full px-3 py-1 text-[10px] font-mono uppercase tracking-widest transition-colors duration-300",
          ...
          <div className="flex items-center gap-3">
            <span className={cn(
              "inline-flex items-center gap-1.5 rounded-full border px-2 py-0.5 text-[9px] font-mono uppercase tracking-[0.2em]",
              activeBadgeTone
            )}>
              <span className={cn(
            ...
            <div className="flex items-baseline gap-2">
              ...
              {(isDemoMode ? demoInitialized : connectionConnected) && changePct !== undefined && changeValue !== undefined ? (
                <span className={cn("text-[11px] font-mono", changePct < 0 ? "text-negative" : "text-positive")}>
        ...
        <div className="flex shrink-0 items-center gap-2">
          {isDemoMode ? (
            <button
              ...
              className={cn(
                "inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-mono font-medium transition-all duration-300 disabled:opacity-60",
                demoInitialized
                  ? "border-border bg-secondary/40 text-muted-foreground hover:bg-secondary hover:text-foreground"
                  : "border-primary/30 bg-primary/10 text-primary hover:bg-primary/20"
              )}
            >
              {demoInitialized ? <RotateCcw className="h-3 w-3" /> : <Plus className="h-3 w-3" />}
              {demoInitialized ? "Reset" : "Setup"}
            </button>
          ) : null}

          <button className="relative rounded-full border border-border bg-secondary/30 p-2 text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground">
            <Bell className="h-3.5 w-3.5" />
            <span className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-primary animate-pulse" />
          </button>

          <div className="flex items-center gap-2 rounded-full border border-border bg-secondary/30 pl-3 pr-1 py-1">
            <span className="text-[10px] font-mono text-muted-foreground">{session.username}</span>
            <div className="flex h-6 w-6 items-center justify-center rounded-full bg-primary/10 ring-1 ring-primary/20">
              <span className="text-[10px] font-mono font-semibold text-primary">{getUserInitials(session.username)}</span>
            </div>
          </div>
          <button
            onClick={onLogout}
            className="rounded-full border border-border px-2.5 py-1 text-[10px] font-mono text-muted-foreground transition hover:bg-secondary hover:text-foreground"
          >
            Logout
          </button>
        </div>
      ...
      {isDemoSetupModalOpen ? (
        <div
          className="fixed inset-0 z-50 bg-background/80 px-4 py-8 backdrop-blur-md"
          onClick={() => {
            setIsDemoSetupModalOpen(false);
            if (!demoInitialized) {
        ...
        >
          <div
            className="mx-auto mt-10 w-full max-w-3xl rounded-2xl border border-border bg-[linear-gradient(180deg,_hsl(var(--card))_0%,_hsl(var(--secondary)/0.55)_100%)] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.35)]"
